            ...
            @auth
                <a href="{{ route('profile.show') }}">
                    <img 
                        ...
                        alt="Profile" 
                        class="w-10 h-10 rounded-full border-2 border-gray-300 cursor-pointer ml-4" />
                </a>
            @endauth
        </div>
    </div>
...
<script>
    ...
    // Prevent opening the dropdown with click outside
    ...
    // Set category name to the dropdown button
    document.querySelectorAll('#dropdownOptions li').forEach(function(option) {
        option.addEventListener('click', function() {
            document.getElementById('dropdownSpan').textContent = option.textContent;
            document.getElementById('category_id').value = option.getAttribute('data-id');
